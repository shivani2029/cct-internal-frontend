...
const BcaCheckDetailsModel = ({
  ...
  const fetchData = async () => {
    try {
      const response = await getAllCheck('type=default');
      const res = response.data.check;
      setData(res);
    } catch (error) {
                        ...
                        return (
                          <Select
                            onValueChange={value => {
                              ...
                              if (selectedForm) {
                                const sectionData = selectedForm.sections.map(
                                  (sections, parentIndex) => {
                                    return {
                                      title: sections.formName || '',
                                      fields: sections.fields.map(
                                        (field, index) => {
                                          setSelectedTypes(prevTypes => ({
                                            ...prevTypes,
                                            [`${parentIndex}-${index}`]:
                                ...
                                form.setValue('sections', sectionData);
                              }
                            }}
...
BcaCheckDetailsModel.propTypes = {
...
export default BcaCheckDetailsModel;
